...
use crate::{check_repeat, MixTexModel, DECODER_BYTES, ENCODER_BYTES, TOKENIZER_STR};
...
impl MixTexOnnx {
    pub fn build(args:StartArgs) -> std::result::Result<Box<dyn MixTexModel>,Box<dyn std::error::Error>> {
        ...
        let decoder_session = decoder_builder
            ...
            .with_intra_threads(args.intra_threads)?
            .with_inter_threads(args.inter_threads)?
            .commit_from_memory(DECODER_BYTES)?;
        Ok(Box::new(MixTexOnnx {
            encoder_session,
            decoder_session,
...
impl MixTexModel for  MixTexOnnx {
    ...
    fn inference_by_step(&self, img: &[f32],mut callback:&dyn Fn(String) -> bool)-> std::result::Result<String, Box<dyn std::error::Error>>
    {
        let check_rate = MAX_LENGTH / 64;
        let mut result_idx = [0_u32; MAX_LENGTH];
